...
function add(){
    if(JSON.parse( localStorage.getItem('todo3')) === null){
        alert('Чтобы начать пользоваться журналом, необходимо добавить задания каждому работнику')
    }else{
        toDoList = JSON.parse( localStorage.getItem('todo3')) ;
        ...
        elem.addEventListener('click' , ()=> {
            deliteBtn = +elem.id
                ...
                if(confirm('точно надо удалить??')){
                    ...
                    localStorage.setItem('todo3', JSON.stringify(newList));
                    window.location.reload();
                }
...
function showCoutTask(){
    ...
    let complited = 0;
    let compliting = 0;
    JSON.parse( localStorage.getItem('todo3')).forEach(item => {
    //   console.log(item);
      if(!item.complite){
        compliting++;
